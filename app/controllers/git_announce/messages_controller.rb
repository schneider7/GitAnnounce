        ...
        if action_done == "submitted"        
          ...
          when 'approved'
            full_message = <<~MESSAGE
              @**#{zulip_name(onwer)}**, a dev reviewer just approved changes on your PR (`#{title}`). 
              Check it out [here](#{link})
            MESSAGE
          when 'changes_requested'
            full_message = "@**#{zilup_name(owner)}**, changes were requested on your PR (`#{title}`). Read the comments [here](#{link})"
          end
        end
